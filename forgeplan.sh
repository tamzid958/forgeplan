...
handle_init_project() {
  ...
  # --- Step 2b: Add OpenProject config and reviewers to config.json ---
  if [[ -f "forgeplan.config.json" ]]; then
    local tmp_config
    tmp_config=$(mktemp)

    # Inject OpenProject connection
    jq --arg url "${OP_BASE_URL:-}" --arg pid "${OP_PROJECT_ID:-}" \
      '. + {openproject: {url: $url, projectId: $pid}}' \
      forgeplan.config.json > "$tmp_config"
    mv "$tmp_config" forgeplan.config.json

    # Inject reviewers if provided
    if [[ -n "${reviewers:-}" ]]; then
      local reviewers_json
      reviewers_json=$(echo "$reviewers" | tr ',' '\n' | jq -R . | jq -sc '.')
      tmp_config=$(mktemp)
      jq --argjson r "$reviewers_json" '. + {reviewers: $r}' forgeplan.config.json > "$tmp_config"
      mv "$tmp_config" forgeplan.config.json
    fi
  fi
...
_init_env_interactive() {
  ...
  local reviewers
  local validation_cmd claude_model
  ...
  # PR reviewers (optional)
  echo ""
  echo "--- PR Creation ---"
  echo "Git host, repo, and base branch are auto-derived from git remote."
  echo "Auth uses CLI tools (gh auth login, glab auth login)."
  echo ""
  ...
  echo "Claude CLI resolves aliases to the latest version automatically."
  ...
  # Store OP URL and project ID for config.json (written in step 2b)
  # These are exported so _init_config_generate and status mapping can use them
  OP_BASE_URL="$op_base_url"
...
# All project config is in forgeplan.config.json
